...
3. **RA is internally inconsistent across sources.** The RA main workbook's Place-of-Service sheet totals only **126 visits** (58 months, Office and Other only), while that same workbook's pivot has a Grand Total of **1,246** and the RA Branded Generic workbook's sheet has **1,283** (72 months, all four settings). The 126-visit sheet appears to be a filtered subset, not the whole RA market. The "~1,283 RA visits" figure quoted elsewhere in this project comes from the Branded Generic sheet, not from this main-workbook sheet.

   **Decided: RA is out of scope for `fact_place_of_service_visits` in Phase 2.** The Silver builder loads Place-of-Service for OA only. Reasoning: `gold_visit_share_monthly`'s Place-of-Service columns (`hospital_visits`, `office_visits`, `other_visits`, `telehealth_visits` — §2.1 of `silver_gold_data_dictionary.md`) are explicitly sourced `for disease_area = 'OA'` only; nothing in Core scope reads an RA Place-of-Service figure. RA's comparison dashboard (§6.3 of `PROPOSAL.md`) is the only future consumer, and it does not exist yet. When it does, the choice between these three inconsistent sources should be made then, with the dashboard's actual requirement in view, not guessed at now to fill a column nothing reads. This is a scope decision, not a deferral of a decision that has to be made today.
